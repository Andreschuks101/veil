...
export function useInvisibleWallet(config: WalletConfig): InvisibleWallet {
    ...
    const setGuardian = async (
        ...
        try {
            ...
            // The simulation returns auth entries with empty credentials; we fill
            // them with the passkey-signed WebAuthn assertion.
            ...
            if (authEntries) {
                for (const parsed of authEntries) {
                    // Only sign entries addressed to this wallet contract
                    ...
                    // Attach the WebAuthn signature to the auth entry credentials
    ...
    return { address, isDeployed, isPending, error, register, deploy, signAuthEntry, login, setGuardian, initiateRecovery, completeRecovery };
}